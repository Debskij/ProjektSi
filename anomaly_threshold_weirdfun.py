...
from weird_gen import weird_gen
...
file_name = "weirdfun"
model = load_model("funmodel.h5")
TIME_STEPS = 30

# %%

data = val = weird_gen(3, 4, 10000, [1, 20])
x_test = []
y_test = []
pom = 0
...
for i in range(len(data) - TIME_STEPS):
    ...
    y_test.append((data[i + TIME_STEPS] - min_value) / (max_value - min_value))
...
y_plot = []
for y1 in y_test:
    y_plot.append(y1 * (max_value - min_value) + min_value)
